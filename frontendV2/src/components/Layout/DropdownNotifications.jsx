...
function DropdownNotifications({
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setHasUnread(res.data.notifications.some((notification) => !notification.read));
      }
    } catch (error) {
  ...
  const readNotification = async () => {
    try {
      await axios.put('/api/v1/notification/markRead', {}, {
        headers: {
          Authorization: `${user?.token}`,
        },
      });
  ...
  return (
    <div className="relative inline-flex">
      ...
      >
        ...
        {
          hasUnread && (<div className="absolute top-0 right-0 w-2.5 h-2.5 bg-rose-500 border-2 border-white dark:border-[#182235] rounded-full"></div>)
        }
      </button>
          ...
          <ul>
            {
              notifications.length > 0 ? (
                notifications.map(notification => (
                  <li key={notification._id} className={`border-b border-slate-200 dark:border-slate-700 last:border-0 ${!notification.read ? 'bg-yellow-100 dark:bg-yellow-900' : ''}`}>
                    <Link
                    ...
                    >
                      <span className="block text-sm mb-2">
                        📣 <span className={`font-medium ${!notification.read ? 'text-slate-800 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}>
